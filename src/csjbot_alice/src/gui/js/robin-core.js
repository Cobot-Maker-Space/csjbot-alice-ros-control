$(document).ready(function () {
  ...
  $(document).on("change", ".openhab-state-switch", function () {
    window.openHabStatePublisher.publish(
      new ROSLIB.Message({ data: $(this).is(":checked") })
    );
  });
  ...
  $(document).on("click", ".btn-confirm-requested", function () {
    ...
    $("#requested")
      ...
      .each(function () {
        parts_speech +=
          $(this).find(".parts-qty").html() +
          " " +
          $(this).find(".parts-name").html() +
          ". ";
      });
    if (parts_speech == "") {
  ...
  window.videoSubscriber.subscribe(function (message) {
    console.log(
      "Received message on " + window.videoSubscriber.name + ": " + message.data
    );
  ...
  window.scanResultSubscriber.subscribe(function (message) {
    console.log(
      "Received message on " +
        window.scanResultSubscriber.name +
        ": " +
        message.data
    );
  ...
  $(".quick-expression").on("click", function () {
    window.expressionPublisher.publish(
      new ROSLIB.Message({ data: $(this).data("expression") })
    );
  });
  ...
  $(document).on("click", ".slam-home", function () {
    window.moveHomePublisher.publish(new ROSLIB.Message());
  });
      ...
      $.each(item, function (name, speechset) {
        if (name == "short") {
          $.each(speechset, function (i, phrase) {
            $(".templates .quick-speak")
              .clone()
              .html(phrase)
              .data("phrase", phrase)
              .appendTo(".speech-options-short");
          });
        }
        if (name == "long") {
          $.each(speechset, function (i, phrase) {
...
function retrieve_parts(location_param, location) {
  ...
  location_param.get(function (value) {
    $.each(value, function (index, part) {
      ...
      part_obj
        .find(".parts-img")
        .attr("src", "media/parts/" + part.id + ".png");
